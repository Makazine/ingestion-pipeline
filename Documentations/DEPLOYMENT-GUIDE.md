...
powershell past clipboard: powershell Get-Clipboard
...
with open('/tmp/test-files/2025-12-21-test$(printf "%04d" $i).ndjson', 'w') as f:
    for j in range(5000):  # Adjust to get ~3.5MB
        record = {
            'id': f'rec_{$i}_{j}',
            'timestamp': datetime.now(timezone.utc-8).isoformat(),
...
### Step 8.3 alternative option to 8.2: Test the Pipeline

```bash
# Get input bucket name
INPUT_BUCKET=$(aws cloudformation describe-stacks \
  --stack-name ndjson-parquet-processor \
  --query 'Stacks[0].Outputs[?OutputKey==`InputBucketName`].OutputValue' \
  --output text)

# Upload test file
aws s3 cp tests/sample_data.ndjson \
  s3://${INPUT_BUCKET}/2025-12-21-test001.ndjson
